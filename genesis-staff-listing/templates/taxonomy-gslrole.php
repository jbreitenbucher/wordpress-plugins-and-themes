...
add_action('genesis_before','gsl_role_loop_setup');
function gsl_role_loop_setup() {

    if ( ! genesis_html5() ) {
        // Customize Before Loop
            remove_action('genesis_before_loop','genesis_do_before_loop' );
            add_action('genesis_before_loop','gsl_role_before_loop');
        ...
        // Customize Post Content
            ...
            add_action('genesis_post_content','gsl_role_post_content');
        ...
        // Customize Post Content
            ...
            add_action( 'genesis_entry_content', 'gsl_role_post_content' );
...
function gsl_role_before_loop() {
    $term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );
    echo '<h1>' . $term->name . '</h1>';
    if( !is_paged() ) {
        echo '<div>' . $term->description . '</div>';
    }
    if( !is_paged() && $term->slug == genesis_get_option('gsl_staff_role', GSL_SETTINGS_FIELD ) )
        {
            if (!genesis_get_option('gsl_staff_schedule', GSL_SETTINGS_FIELD ) == '') {
            echo '<br />';
            echo '<iframe width="550" height="325" frameborder="0" src="https://docs.google.com/spreadsheet/pub?hl=en_US&hl=en_US&key='.  genesis_get_option('gsl_staff_schedule', GSL_SETTINGS_FIELD ) . '&output=html&widget=true"></iframe>';
            }
        }
...
function gsl_role_post_content () {
    global $post, $c;
