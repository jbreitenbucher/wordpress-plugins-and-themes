    ...
    class blog_templates {
        ...
        */
        ...
        function __construct() {
            ...
            // Always register the Network Admin menu hook; WordPress will
            // only fire it in the Network Admin, so we don't need to gate
            // this on is_network_admin() (which may not be reliable at load time).
            add_action( 'network_admin_menu', array( $this, 'register_admin_menus' ) );



            add_action( 'init', array( &$this, 'maybe_upgrade' ) );
            add_action( 'init', array( &$this, 'init' ) );
            add_action( 'wp_initialize_site', array( $this, 'handle_wp_initialize_site' ), 9999, 2 );
        ...
        function add_template_selector() {
			?>
        ...
        */
        function set_blog_defaults( $blog_id, $user_id, $_passed_domain=false, $_passed_path=false, $_passed_site_id=false, $_passed_meta=false ) {
            ...
            if ( class_exists( $classname ) ) {
                ...
                $copier = new $classname( $source_blog_id, $blog_id, $user_id, $copier_args );
                $copier->execute();
            }
        ...
        */
        ...
        /**
         * Register Network Admin menus when admin menu system is loaded.
         * Ensure admin menu classes are loaded here so they exist when instantiated.
         */
            function register_admin_menus() {
                if ( is_network_admin() ) {
                 ...
                 $main_menu->network_admin_page();
                    ...
                    $settings_menu = new blog_templates_settings_menu();
                    $settings_menu->network_admin_page();
                }
            }

        /**
         * Handle modern multisite site initialization.
         */
        public function handle_wp_initialize_site( $new_site, $args ) {
            if ( ! ( $new_site instanceof \WP_Site ) ) {
                return;
            }
            $blog_id = (int) $new_site->blog_id;
            $user_id = ( is_array( $args ) && isset( $args['user_id'] ) )
                ? (int) $args['user_id']
                : ( function_exists( 'get_current_user_id' ) ? (int) get_current_user_id() : 0 );

            $domain  = $new_site->domain;
            $path    = $new_site->path;
            $site_id = (int) $new_site->site_id;
            $meta    = ( is_array( $args ) && isset( $args['meta'] ) ) ? $args['meta'] : array();

            $flag_key = 'nbt_initialized_' . $blog_id;
            if ( ! get_site_option( $flag_key ) ) {
                $this->set_blog_defaults( $blog_id, $user_id, $domain, $path, $site_id, $meta );
                update_site_option( $flag_key, time() );
            }
        }
