...
function gsl_create_metaboxes( $meta_boxes ) {
    $prefix = 'gsl_'; // start with an underscore to hide fields from custom fields list
    $meta_boxes[] = array(
        'id' => 'staff_info_metabox',
        'title' => 'Information',
        'pages' => array('gslstaff'), // post type
        'context' => 'normal',
        'priority' => 'low',
...
function gsl_initialize_cmb_meta_boxes() {
    if ( !class_exists( 'cmb_Meta_Box' ) ) {
        require_once( plugin_dir_path( __FILE__ ) . '../../lib/metabox/init.php' );
    }
}

add_action( 'init', 'gsl_initialize_cmb_meta_boxes', 9999 );
