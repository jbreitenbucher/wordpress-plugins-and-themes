(function(){
	...
	function wireToggle(toc){
		if (!toc) return;
		var btn = toc.querySelector('.ibp-toc__toggle');
		var contentId = toc.getAttribute('data-content');
		var panel = contentId ? document.getElementById(contentId) : toc.querySelector('.ibp-toc__content');
		if (!btn || !panel) return;
	...
	function init(){
		document.querySelectorAll('.ibp-toc').forEach(function(toc){
			ensureIds(toc);
			wireToggle(toc);
		});
	}
