	...
	registerBlockType('inside-block-patterns/toc', {
		...
		edit(props) {
			...
			const headings = collectHeadings(blocks)
				...
				.filter((h) => {
					if (h.level === 2) return true; // always include H2
			...
			const base = 2;
			const blockProps = useBlockProps({ className: 'ibp-toc' });

			function renderList(items) {
				let currentLevel = base;
				const stack = [ { type: 'ul', props: { className: 'ibp-toc__list level-' + base }, children: [] } ];
				let currentUl = stack[0];

				items.forEach((h, idx) => {
					const level = Math.min(Math.max(h.level, 2), 6);
					let diff = level - currentLevel;
					...
					while (diff < 0) {
						// move up
					 // we don't have back refs, but since preview is lightweight, restart structure isn't worth it
						// leave as is; preview nesting will be correct from incremental build
						diff++;
						currentLevel--;
					}
			...
			return el(Fragment, {},
				el(InspectorControls, {},
					el(PanelBody, { title: 'Table of contents', initialOpen: true },
						...
						el(ToggleControl, { label: 'Collapse TOC by default', checked: collapsed, onChange: (v) => setAttributes({ collapsed: v }) }),
						el(RangeControl, {
							label: 'Max depth',
							min: 2, max: 6,
							value: maxDepth,
						...
						el(Notice, { status: 'info', isDismissible: false },
							'Include toggles below let you omit specific levels up to the max depth.'
						),
