...
/**
 * Safely extract a user's DN from an LDAP search result.
 *
 * In PHP 8+, ldap_get_dn() throws a TypeError if given `false` instead of a valid entry.
 * Older versions often used `@` to suppress warnings, but that does not prevent TypeErrors.
 *
 * @param mixed $connection LDAP connection (resource in older PHP, LDAP\Connection in newer PHP).
 * @param mixed $results    LDAP search result (resource in older PHP, LDAP\Result in newer PHP).
 * @return string|false     Distinguished Name (DN) or false if not available.
 */
function wpDirAuth_get_dn_from_search_results($connection, $results) {
    if (empty($results)) {
        return false;
    }
            ...
            if ($preBindUser && $preBindPassword) {
                ...
                if ( $isPreBound = wpDirAuth_bindTest($connection, $preBindUser, $preBindPassword,$baseDn) === true ) {
                    if ( ($results = @ldap_search($connection, $baseDn, $filterQuery, $returnKeys)) !== false ) {
                                            $userDn = wpDirAuth_get_dn_from_search_results($connection, $results);
                        if ( $userDn !== false ) {
...
________EOS;
    ...
    if (function_exists('add_action')) {
        ...
        /**
         * Print the password-change message on the lost password screen.
         *
         * Note: create_function() was removed in PHP 8; use a closure.
         */
