...
add_action('genesis_before','gsl_expertise_loop_setup');
function gsl_expertise_loop_setup() {

    if ( ! genesis_html5() ) {
        // Customize Before Loop
            remove_action('genesis_before_loop','genesis_do_before_loop' );
            add_action('genesis_before_loop','gsl_expertise_before_loop');
        ...
        // Customize Post Content
            ...
            add_action('genesis_post_content','gsl_expertise_post_content');
...
function gsl_expertise_before_loop() {
...
function gsl_expertise_post_content () {
    global $post, $c;
    ...
    printf( '<div id="post-%s" class="person clear">', get_the_ID() );
        ...
        if (genesis_get_custom_field('gsl_title_text') != '') {
            printf( '<h1 class="name"><a href="%s" title="%s">%s</a>, <span class="title">%s</span></h1>', get_permalink(), the_title_attribute('echo=0'), get_the_title(), genesis_get_custom_field('gsl_title_text') );
        } else {
            printf( '<h1 class="name"><a href="%s" title="%s">%s</a></h1>', get_permalink(), the_title_attribute('echo=0'), get_the_title() );
                ...
                echo '<div class="contact clear">';
                    if( genesis_get_custom_field('gsl_phone_number_text') != '' && genesis_get_custom_field('gsl_email_address_text') == '') {
                        printf( '<span class="phone">phone: %s</span>', genesis_get_custom_field('gsl_phone_number_text') );
                    }
                    if( genesis_get_custom_field('gsl_email_address_text') != '' && genesis_get_custom_field('gsl_phone_number_text') == '') {
                    ...
                    if( genesis_get_custom_field('gsl_email_address_text') != '' && genesis_get_custom_field('gsl_phone_number_text') != '') {
                        printf('<span class="phone">phone: %s</span> | <span class="email">e-mail: <a href="mailto:%s">%s</a></span>', genesis_get_custom_field('gsl_phone_number_text'), antispambot(genesis_get_custom_field('gsl_email_address_text')), antispambot(genesis_get_custom_field('gsl_email_address_text')) );
                    }
