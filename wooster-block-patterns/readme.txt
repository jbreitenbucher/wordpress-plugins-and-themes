...
- **Custom Blocks (Wooster Blocks category)**
  - **Accordion (`wbp/accordion`)** and **Accordion Item (`wbp/accordion-item`)**
...
- **Patterns (grouped under Wooster- prefixed categories in the Patterns panel)**
  - **Department home**
...
- Patterns use an `.wbp-container` constrained wrapper and minimal CSS (`assets/css/wbp.css`) for intrinsic, responsive layouts.
...
- **Blocks:** appear under **Wooster Blocks** (slug `wbp-content`) in the block inserter.
- **Patterns:** appear under:
  - Wooster Department & Office (`wbp-department`)
  - Wooster Content Layouts (`wbp-content`)
  - Wooster Messaging & Alerts (`wbp-messaging`)
  - Wooster People & Directory (`wbp-people`)
  - Wooster News & Posts (`wbp-news`)

== Installation ==
1. Upload the `wooster-block-patterns` folder to `/wp-content/plugins/`.
2. Activate the plugin in **Plugins → Installed Plugins**.
3. In the editor:
   - Open the **Blocks** tab to find the **Wooster Blocks** category (Accordion, Accordion Item, TOC).
   - Open the **Patterns** tab to find Wooster-prefixed pattern categories.
...
- **Accordion** (`wbp/accordion`, `wbp/accordion-item`): static blocks. Editor JS ensures the item has a stable `uid` when needed and mirrors front-end behavior without breaking validation.
- **TOC** (`wbp/toc`): **dynamic** block with `render_callback` for nested lists. Editor shows a robust preview with a separate collapse state. Attributes:  
...
* **Fix:** Blocks picker category now **Wooster Blocks** (slug `wbp-content`). Removed slug mismatch that caused “Uncategorized.”
...
* **Polish:** Department home (about + services + news + contact) width/containers corrected with `alignfull` + inner `alignwide wbp-container`.
...
* **CSS:** Consolidated utility classes in `assets/css/wbp.css` and enqueued in editor + front end with cache-busting.
